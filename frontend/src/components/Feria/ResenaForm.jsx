...
const ResenaForm = ({ feriaId, onResenaCreated, resena, onCancel }) => {
    ...
    const [metodosPago, setMetodosPago] = useState(
        resena
            ? (typeof resena.metodosPago === 'string' ? JSON.parse(resena.metodosPago) : resena.metodosPago)
            : { efectivo: true, tarjeta: false }
    )
    const [enviando, setEnviando] = useState(false)

    const [variedad, setVariedad] = useState(
        resena
            ? (typeof resena.variedad === 'string' ? JSON.parse(resena.variedad) : resena.variedad)
            : {
    ...
    const [afluenciaPorBloque, setAfluenciaPorBloque] = useState(
        resena
            ? (typeof resena.afluenciaDetallada === 'string' ? JSON.parse(resena.afluenciaDetallada) : resena.afluenciaDetallada)
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            toast.error(resena ? 'Error al actualizar' : 'Error al publicar')
        } finally {
            setEnviando(false)
        }
    }
    ...
    return (
        <form onSubmit={handleSubmit} className="bg-gray-50 p-5 rounded-xl border shadow-sm mb-6 space-y-6">
            <h3 className="font-bold text-gray-800 text-base border-b pb-2">
                {resena ? 'Editar tu reseña' : 'Tu experiencia en la feria'}
            </h3>
            ...
            <div className="flex gap-3">
                ...
                <button
                    type="submit"
                    disabled={enviando}
                    className={`flex-1 bg-green-600 text-white py-3 rounded-lg font-bold hover:bg-green-500 transition-colors ${!onCancel && 'w-full'}`}
                >
                    {enviando ? (resena ? 'Guardando...' : 'Publicando...') : (resena ? 'Guardar Cambios' : 'Publicar Reseña')}
                </button>
            </div>
        </form>
    )
