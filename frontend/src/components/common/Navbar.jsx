...
export default function Navbar() {
    ...
    // Mantenemos useQuery para que el contador sea reactivo y se sincronice al instante
    const { data: pendingFeriantes = [] } = useQuery({
        queryKey: ['pending-count'],
        queryFn: async () => {
            const res = await api.get('/feriantes?estado=pendiente')
            return res.data
        },
        enabled: user?.rol === 'admin',
        refetchInterval: 60000 // Actualización de respaldo cada minuto
    })

    const pendingCount = pendingFeriantes.length
        ...
        <header className="bg-white border-b border-gray-100 shadow-sm sticky top-0 z-50">
            <nav className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
                ...
                <div className="hidden md:flex items-center gap-6">
                    ...
                    {/* LINK DE SOLICITUDES CON EL CONTADOR AL LADO */}
                    {user?.rol === 'admin' && (
                        <NavLink
                ...
                <div className="md:hidden border-t border-gray-100 bg-white px-4 py-3 flex flex-col gap-3">
                    ...
                    {user?.rol === 'admin' && (
                        <NavLink
                            ...
                            onClick={() => setMenuOpen(false)}
                            className={({ isActive }) =>
                                `flex items-center justify-between text-sm font-medium py-1 ${isActive ? 'text-primary-600' : 'text-gray-600'}`
                            }
                        >
                            Solicitudes
                            {pendingCount > 0 && (
